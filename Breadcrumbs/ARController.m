...
- (void)deviceOrientationDidChange:(NSNotification *)notification {
	...
	if ( orientation != UIDeviceOrientationUnknown && 
		...
		CGAffineTransform transform = CGAffineTransformMakeRotation(DEG_TO_RAD(0));
		CGRect bounds = [[UIScreen mainScreen] bounds];
		[app setStatusBarHidden:YES];
		[app setStatusBarOrientation:UIInterfaceOrientationPortrait animated: NO];
		
		if (orientation == UIDeviceOrientationLandscapeLeft) {
			transform		   = CGAffineTransformMakeRotation(DEG_TO_RAD(90));
			...
			transform = CGAffineTransformMakeRotation(DEG_TO_RAD(180));
...
- (void)updateCurrentCoordinate {
	...
	if (self.currentOrientation == UIDeviceOrientationLandscapeLeft)
		adjustment = DEG_TO_RAD(270); 
	else if (currentOrientation == UIDeviceOrientationLandscapeRight)
		adjustment = DEG_TO_RAD(90);
	else if (currentOrientation == UIDeviceOrientationPortraitUpsideDown)
		adjustment = DEG_TO_RAD(180);

	//NSLog(@"updateCurrentCoordinate: (1) adjustment = %.3f, currentCoordinate.azimuth = %.3f", RAD_TO_DEG(adjustment), RAD_TO_DEG( self.currentCoordinate.coordinateAzimuth ) );
	
	self.currentCoordinate.coordinateAzimuth =  
	     DEG_TO_RAD(self.currentHeading.magneticHeading) - adjustment;

	//NSLog(@"updateCurrentCoordinate: (2) adjustment = %.3f, currentCoordinate.azimuth = %.3f, magneticHeading = %3.f", RAD_TO_DEG(adjustment), RAD_TO_DEG( self.currentCoordinate.coordinateAzimuth ), self.currentHeading.magneticHeading );
...
- (void)updateLocations {
	...
	//NSLog(@"updateLocations: viewAngle %.3f, currentCoordinate.azimuth %.3f", -RAD_TO_DEG(self.viewAngle), RAD_TO_DEG(self.currentCoordinate.coordinateAzimuth));
...
- (BOOL)viewportContainsCoordinate:(ARCoordinate *)coordinate {
	...
	BOOL result	= NO;
	if (deltaAzimuth <= DEG_TO_RAD(self.viewRange)) {
		result = YES;
	}
	//NSLog(@"viewPortContainsView: location = %@, viewRange %.3f, deltaAzimuth %.3f", coordinate.coordinateTitle, self.viewRange, RAD_TO_DEG(deltaAzimuth));
...
- (double)deltaAzimuthForCoordinate:(ARCoordinate *)coordinate {
	...
	//	  RAD_TO_DEG(currentAzimuth), RAD_TO_DEG(pointAzimuth));
	...
	if (currentAzimuth < DEG_TO_RAD(self.viewRange) && 
		pointAzimuth > DEG_TO_RAD(360-self.viewRange)) {
		deltaAzimith	= (currentAzimuth + ((M_PI * 2.0) - pointAzimuth));
	} else if (pointAzimuth < DEG_TO_RAD(self.viewRange) && 
			   currentAzimuth > DEG_TO_RAD(360-self.viewRange)) {
...
- (CGPoint)pointForCoordinate:(ARCoordinate *)coordinate {
	...
	//NSLog(@"pointForCoordinate: location = %@, pointAzimuth = %.3f, pointInclination = %.3f, currentAzimuth = %.3f", coordinate.coordinateTitle, point.x, point.y, RAD_TO_DEG(pointAzimuth), RAD_TO_DEG(currentAzimuth), RAD_TO_DEG(pointInclination) );
		
	double deltaAzimuth = [self deltaAzimuthForCoordinate:coordinate];
	BOOL isBetweenNorth	= [self isNorthForCoordinate:coordinate];
	
	//NSLog(@"pointForCoordinate: (1) currentAzimuth = %.3f, pointAzimuth = %.3f, isNorth = %d", RAD_TO_DEG(currentAzimuth), RAD_TO_DEG(pointAzimuth), isBetweenNorth );
	
	//NSLog(@"pointForCoordinate: deltaAzimuth = %.3f", RAD_TO_DEG(deltaAzimuth));
	//NSLog(@"pointForCoordinate: (2) currentAzimuth = %.3f, pointAzimuth = %.3f, isNorth = %d", RAD_TO_DEG(currentAzimuth), RAD_TO_DEG(pointAzimuth), isBetweenNorth );
	
	if ((pointAzimuth > currentAzimuth && !isBetweenNorth) || 
		(currentAzimuth > DEG_TO_RAD(360-self.viewRange) && 
		 pointAzimuth < DEG_TO_RAD(self.viewRange))) {
			
		// Right side of Azimuth			
		point.x = (viewBounds.size.width / 2) + ((deltaAzimuth / DEG_TO_RAD(1)) * 12);  
	} else {
	...
	point.y = (viewBounds.size.height / 2) 
	          + (RAD_TO_DEG(M_PI_2 + self.viewAngle)  * 2.0)
	          + ((pointInclination / DEG_TO_RAD(1)) * 12);
	
	//NSLog(@"pointForCoordinate: location = %@, point.x = %.3f, point.y = %.3f, pointAzimuth = %.3f, currentAzimyth = %.3f, viewAngle = %.3f, deltaAzimuth = %.3f, viewBounds.size( width = %.3f, height = %.3f )", coordinate.coordinateTitle, point.x, point.y, RAD_TO_DEG(pointAzimuth), RAD_TO_DEG(currentAzimuth), self.viewAngle, deltaAzimuth, viewBounds.size.width, viewBounds.size.height );
